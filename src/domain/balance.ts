...
type Balance = { credits: Credit[], debits: Debit[], sumOfCashInFlow: number, sumOfCashOutflow: number, netChangeInCash: number }

const balanceCons = (credits: Credit[] = [], debits: Debit[] = [], sumOfCashInFlow: number = 0, sumOfCashOutflow: number = 0, netChangeInCash: number = 0): Balance => {
    return {
        credits: credits,
        debits: debits,
...
export const simplifyGroupExpenses = (group: Group): Group => {
    const expenses = group.expenses.reduce((previousExpenses, currentExpense) => {
        const previousAmount = (previousExpenses?.[currentExpense.debtor]?.[currentExpense.creditor] ?? 0);
    ...
    for (var debtor in expenses) {
        for (var creditor in expenses[debtor]) {
            ...
            if (debtorShouldGiveToCreditor > creditorShouldGiveInReturn) {
                simplifiedExpenses.push({creditor: creditor, amount: (debtorShouldGiveToCreditor - creditorShouldGiveInReturn), debtor: debtor})
            }
        }
    }
...
export const calculateBalances = (group: Group): Balances => {
    ...
    return group.people.reduce((acc, people) => {
        const credits = simplifiedGroup.expenses.filter(e => e.creditor === people).map(c => { return { amount: c.amount, debtor: c.debtor } });
        const debits = simplifiedGroup.expenses.filter(e => e.debtor === people).map(d => { return { creditor: d.creditor, amount: d.amount } })
...
export const giversAndReceivers = (balances: Balances) => {
    return {
        giver: filterUsingKey(balances, (key, balance) => balance.netChangeInCash > 0),
        receiver: filterUsingKey(balances, (key, balance) => balance.netChangeInCash < 0),
    }
}